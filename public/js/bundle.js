    ...
    didMount() {
        this.props.children.forEach((element)=>{
            ...
            for(let i = 0; i < el.length; i++){
                ...
                for(let attr2 = 0; attr2 < el[i].attributes.length; attr2++){
                    attrs2.push(el[i].attributes.item(attr2)?.name.toLowerCase());
                    attrs2.push(el[i].attributes.item(attr2)?.value.toLowerCase());
                }
                attrs2 = attrs2.sort();
            ...
            render() {
                ...
                styles.forEach((style)=>{
                    if (typeof style === 'string') {
                        ...
                        const _style1 = style.toString?.();
                        if (typeof _style1 === 'string') {
                            helmets.push(h(Helmet, null, h('style', null, _style1)));
                        }
                    }
                });
                const component = children && children.length > 0 ? h(WrappedComponent, {
                    ...rest
                }, children) : h(WrappedComponent, {
                    ...this.props
                });
...
class Platform extends Component {
    ...
    keyBoard(keymap) {
        ...
        document.addEventListener("keydown", ({ key  })=>keymap[key] && pressing.add(keymap[key]));
        document.addEventListener("keyup", ({ key  })=>keymap[key] && pressing.delete(keymap[key]));
        return ()=>pressing;
    }
    move(now, direction) {
        now *= 0.001;
...
const __default1 = withStyles(CSS)(Platform);
...
const __default2 = withStyles(SceneCSS)(Scene);
const el = document.getElementById('scene');
if (el) hydrate(h(__default2, null), el);
const el1 = document.getElementById('platform');
if (el1) hydrate(h(__default1, null), el1);
